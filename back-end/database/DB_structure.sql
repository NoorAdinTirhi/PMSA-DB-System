...
SELECT UniID, CONCAT(FirstName, " ", FatherName, " ",GFatherName, " ",FamilyName) AS Name FROM M WHERE CONCAT(FirstName, " ", FatherName, " ",GFatherName, " ",FamilyName) LIKE '%%' AND UniID NOT IN (SELECT UniID FROM M_A WHERE ActivityID = 11) AND M.LC = IFNULL((SELECT La.LC FROM La WHERE La.ActivityID = 1), M.LC);


DELIMITER ;

SELECT La.LC FROM La WHERE ActivityID = 11

DELETE FROM M WHERE UniID = 1190082 AND ('nablus' = 'national' OR 'nablus' = (SELECT La.LC FROM La WHERE ActivityID = 11));

INSERT INTO M_A SELECT 1190083, 12, 0, 'Organizer'  WHERE ('undefined' = 'national' OR 'undefined' = (SELECT La.LC FROM La WHERE ActivityID = 12))
SELECT * FROM M;

SELECT 1190081,12,0,'Organizer'